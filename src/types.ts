...
export interface MergedProjectData extends ProjectData {
  ...
  events: ActivityEvent[];          // L3: all tool events (chronological, isPlanning tag for L2 events)
...
export interface ActivityEvent {
  ...
  isPlanning?: boolean;  // true if toolName ∈ PLANNING_TOOLS (L2 decision event)
}
...
export interface RoadmapData {
  ...
  unrecognized?: boolean; // file exists but has no parseable checkboxes
  lineCount?: number;     // hint for UI when unrecognized
}

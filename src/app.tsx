...
// Build activity entries from all projects
type ActivityEntry = {
  projectName: string;
  label: string;
  status: string;
    ...
    <Panel title={title} hotkey={isInner ? undefined : "2"} flexGrow={1} focused={isInner || panelFocused}>
      ...
      {(() => {
        ...
        const renderTask = (t: DisplayTask, idx: number) => {
          const isCursor = isInner && idx === taskIdx;
          const isGone = !!t.gone;
      ...
      {isInner && project.tasks[taskIdx] && !project.tasks[taskIdx].gone && (
        <>
          <Text> </Text>
